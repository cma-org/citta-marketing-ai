...
const TestimonialsSection = () => {
  ...
  return <section id="testimonials" className="py-20 md:py-32 bg-gradient-to-b from-blue-50 to-white overflow-hidden">
      <div className="container px-4 sm:px-6 lg:px-8 mx-auto">
        ...
        <div className="relative max-w-4xl mx-auto reveal">
          <div className="glass-panel rounded-2xl p-8 md:p-12">
            {testimonials.map((testimonial, index) => <div key={index} className={`transition-all duration-500 ease-smooth ${index === activeIndex ? 'opacity-100 translate-x-0' : 'opacity-0 absolute top-0 left-0 translate-x-8'}`} style={{
            display: index === activeIndex ? 'block' : 'none'
                ...
                <div className="flex flex-col items-center text-center">
                  ...
                  <div className="flex flex-col items-center">
                    <img src={testimonial.avatar} alt={testimonial.author} className="w-16 h-16 rounded-full object-cover mb-4" />
          ...
          <div className="flex justify-center mt-10 space-x-4">
            <Button onClick={prevTestimonial} variant="outline" size="icon" className="rounded-full h-12 w-12 border border-gray-300 hover-card-animation">
              <ChevronLeft className="h-5 w-5" />
            </Button>
            
            <Button onClick={nextTestimonial} variant="outline" size="icon" className="rounded-full h-12 w-12 border border-gray-300 hover-card-animation">
          ...
          <div className="flex justify-center mt-6 space-x-2">
            {testimonials.map((_, index) => <button key={index} onClick={() => setActiveIndex(index)} className={`w-2 h-2 rounded-full transition-all ${index === activeIndex ? 'bg-blue-600 w-6' : 'bg-gray-300'}`} aria-label={`Go to testimonial ${index + 1}`} />)}
          </div>
        </div>
      </div>
    </section>;
